...
namespace Authorization.Api.Controllers
{
    ...
    /// </summary>
    [SecurityHeaders]
    public class AccountController : MultitenantBaseController
    {
        ...
        private readonly RecoverPasswordEmailSettings _recoverPasswordEmailSettings;
        ...
        public AccountController(
            ...
            IOptions<RecoverPasswordEmailSettings> recoverPasswordEmailSettings) : base(httpContextAccessor)
        {
            ...
            _recoverPasswordEmailSettings = recoverPasswordEmailSettings.Value;
        }
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    var result = await _signInManager.PasswordSignInAsync(user.GetUsername(tenant.Name, model.Email), model.Password, model.RememberLogin, lockoutOnFailure: true);
                    if (result.Succeeded)
                    {
                        //var user = await _userManager.FindByEmailAsync(model.Email);
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordInputModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                _recoverPasswordEmailSettings.Body = GetRecoverPasswordEmailBody(callbackUrl);
                EmailSender.SendRecoverPasswordEmail(user.Email, _recoverPasswordEmailSettings, _smtpSetting);
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordInputModel model)
        {
            ...
            {
                ModelState.AddModelError("", AccountOptions.PasswordMatchErrorMessage);
                return View();
            }
        ...
        private string GetRecoverPasswordEmailBody(string recoveryLink)
        {
            string path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Views", "EmailTemplates", "RecoverPasswordEmailTemplate.htm");
